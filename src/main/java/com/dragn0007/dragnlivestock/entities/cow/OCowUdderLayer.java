...
public class OCowUdderLayer extends GeoRenderLayer<OCow> {
    public OCowUdderLayer(GeoRenderer entityRendererIn) {
    ...
    public void render(PoseStack poseStack, OCow animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        RenderType renderMarkingType = RenderType.entityCutout(((OCow)animatable).getUddersLocation());
        poseStack.pushPose();
