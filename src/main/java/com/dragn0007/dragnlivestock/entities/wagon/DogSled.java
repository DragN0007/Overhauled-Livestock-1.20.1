...
public class DogSled extends AbstractInventoryWagon {

    public static final Vec3[] RIDERS = new Vec3[] {
            new Vec3(0, 1.1D, -1.6D),
            new Vec3(-0.4, 0.55D, 0.5D)
    };

    public static final Vec3[] ANIMALS = new Vec3[] {
            new Vec3(-0.7D, 0, 2.0D),
            new Vec3(0.7D, 0, 2.0D),
            new Vec3(-0.7D, 0, 4.0D),
            new Vec3(0.7D, 0, 4.0D),
            new Vec3(-0.7D, 0, 6.0D),
            new Vec3(0.7D, 0, 6.0D)
    };

    public DogSled(EntityType<? extends AbstractWagon> type, Level level) {
        super(type, level, 0.42D, 2.0D, 3.0F, 40, 36, ANIMALS, 1.25D, 1.25D, RIDERS);
    }
    ...
    public ItemStack getPickResult() {
        return LOItems.DOG_SLED.get().getDefaultInstance();
    }
