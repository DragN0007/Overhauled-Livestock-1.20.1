...
public class FloweringMoobloomRender extends GeoEntityRenderer<FloweringMoobloom> {
    ...
    public FloweringMoobloomRender(EntityRendererProvider.Context renderManager) {
        ...
        this.addRenderLayer(new FloweringMoobloomBodyLayer(this));
        this.addRenderLayer(new FloweringMoobloomTackLayer(this));
    }

    @Override
    public void preRender(PoseStack poseStack, FloweringMoobloom animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
