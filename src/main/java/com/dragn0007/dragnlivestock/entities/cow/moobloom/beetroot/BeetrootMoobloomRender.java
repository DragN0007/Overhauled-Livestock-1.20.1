...
public class BeetrootMoobloomRender extends GeoEntityRenderer<BeetrootMoobloom> {
    ...
    public BeetrootMoobloomRender(EntityRendererProvider.Context renderManager) {
        ...
        this.addRenderLayer(new BeetrootMoobloomBodyLayer(this));
        this.addRenderLayer(new BeetrootMoobloomTackLayer(this));
    }

    @Override
    public void preRender(PoseStack poseStack, BeetrootMoobloom animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
