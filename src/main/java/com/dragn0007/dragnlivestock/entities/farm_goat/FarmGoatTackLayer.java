...
@OnlyIn(Dist.CLIENT)
public class FarmGoatTackLayer extends GeoRenderLayer<FarmGoat> {
    public FarmGoatTackLayer(GeoRenderer<FarmGoat> entityRendererIn) {
    ...
    public void render(PoseStack poseStack, FarmGoat animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if (!animatable.hasChest() && !animatable.isCollared()) return;
        ...
        if (animatable.isCollared()) {
            DyeColor dyeColor = animatable.getCollarColor();
            if (dyeColor != null) {
                resourceLocation = new ResourceLocation(LivestockOverhaul.MODID, "textures/entity/farm_goat/collar/" + dyeColor + ".png");
            }
        }
    }
}
