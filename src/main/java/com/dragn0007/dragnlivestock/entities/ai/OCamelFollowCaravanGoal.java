   ...
   public boolean canUse() {
      if (!this.llama.isLeashed() && !this.llama.inCaravan()) {
         List<Entity> list = this.llama.level().getEntities(this.llama, this.llama.getBoundingBox().inflate(9.0D, 4.0D, 9.0D), (p_25505_) -> {
            ...
            return entitytype == EntityTypes.O_CAMEL_ENTITY.get();
         });
         OCamel llama = null;
         double d0 = Double.MAX_VALUE;

         for(Entity entity : list) {
            OCamel llama1 = (OCamel)entity;
   ...
   public void tick() {
      if (this.llama.inCaravan()) {
         if (!(this.llama.getLeashHolder() instanceof LeashFenceKnotEntity)) {
            OCamel llama = this.llama.getCaravanHead();
   ...
   private boolean firstIsLeashed(OCamel p_25507_, int p_25508_) {
      if (p_25508_ > 8) {
         ...
         if (p_25507_.getCaravanHead().isLeashed()) {
            ...
            OCamel llama = p_25507_.getCaravanHead();
            ++p_25508_;
