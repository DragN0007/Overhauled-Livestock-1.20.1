...
public class GoodsCart extends AbstractInventoryWagon {

    public static final Vec3[] RIDERS = new Vec3[] {
            new Vec3(0, 1.3D, 1.55D)
    };

    public static final Vec3[] ANIMALS = new Vec3[] {
            new Vec3(0.0D, 0, 3.3D)
    };

    public GoodsCart(EntityType<? extends AbstractWagon> type, Level level) {
        super(type, level, LivestockOverhaulCommonConfig.GOODS_CART_SPEED_MULT.get(), 2.0D, 3.0F, 40, 36, ANIMALS, 1.25D, 1.25D, RIDERS);
    }
    ...
    public ItemStack getPickResult() {
        return LOItems.GOODS_CART.get().getDefaultInstance();
    }
