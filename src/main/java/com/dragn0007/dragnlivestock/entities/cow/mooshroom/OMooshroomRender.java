...
public class OMooshroomRender extends GeoEntityRenderer<OMooshroom> {
    ...
    public OMooshroomRender(EntityRendererProvider.Context renderManager) {
        ...
        this.addRenderLayer(new OMooshroomBodyLayer(this));
        this.addRenderLayer(new OMooshroomMushroomLayer(this));
        this.addRenderLayer(new OMooshroomTackLayer(this));
    }

    @Override
    public void preRender(PoseStack poseStack, OMooshroom animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
