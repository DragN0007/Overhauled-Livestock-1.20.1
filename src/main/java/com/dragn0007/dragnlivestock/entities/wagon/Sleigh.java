...
public class Sleigh extends AbstractInventoryWagon {

    public static final Vec3[] RIDERS = new Vec3[] {
            new Vec3(0, 1.5D, 0.6D),
            new Vec3(0.4, 1.3D, -1.9D),
            new Vec3(-0.4, 1.3D, -1.9D),
            new Vec3(0.4, 1.3D, 0D),
            new Vec3(-0.4, 1.3D, 0D)
    };

    public static final Vec3[] ANIMALS = new Vec3[] {
            new Vec3(-0.7D, 0, 3.0D),
            new Vec3(0.7D, 0, 3.0D),
            new Vec3(-0.7D, 0, 7.0D),
            new Vec3(0.7D, 0, 7.0D)
    };

    public Sleigh(EntityType<? extends AbstractWagon> type, Level level) {
        super(type, level, LivestockOverhaulCommonConfig.SLEIGH_SPEED_MULT.get(), 2.0D, 3.0F, 80, 18, ANIMALS, 1.25D, 1.25D, RIDERS);
    }
    ...
    public ItemStack getPickResult() {
        return LOItems.SLEIGH.get().getDefaultInstance();
    }
