...
public class OCowRender extends GeoEntityRenderer<OCow> {
    ...
    public OCowRender(EntityRendererProvider.Context renderManager) {
        ...
        this.addRenderLayer(new OCowBodyLayer(this));
        this.addRenderLayer(new OCowTackLayer(this));
    }

    @Override
    public void preRender(PoseStack poseStack, OCow animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
